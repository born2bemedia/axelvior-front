...
function TabClarity({ t }: { t: ReturnType<typeof useTranslations> }) {
  return (
...
function TabStructure({ t }: { t: ReturnType<typeof useTranslations> }) {
  return (
    <div className={styles.why_tabs__tab_content}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
...
function TabGrowth({ t }: { t: ReturnType<typeof useTranslations> }) {
  const items = [
    {
      icon: imgIconGps,
      key: 'item1',
    ...
    },
    {
      ...
      descFallback: 'that make it easier to scale, not just increasing capacity',
    },
    {
      icon: imgIconArrow,
      key: 'item3',
    ...
    <>
      <div className={styles.why_tabs__tab_content}>
        ...
        <div className={styles.why_tabs__tab_text}>
          <p>
            {t('tab3.desc1', {
              fallback: "We don't believe in growth for growth's sake.",
            })}
          </p>
          <p>
            {t('tab3.desc2', {
          ...
          {items.map((item) => (
            <div key={item.key} className={styles.why_tabs__list_item}>
              ...
              <div className={styles.why_tabs__list_item_text}>
                <strong>{item.titleFallback}</strong>
                <span>{item.descFallback}</span>
              </div>
            </div>
          ))}
        </div>
      ...
      <p className={styles.why_tabs__tab_footer}>
        {t('tab3.footer', {
          fallback: 'Intentional growth is about doing things with purpose, not just doing more.',
        })}
      </p>
    </>
  );
...
function TabCommitment({ t }: { t: ReturnType<typeof useTranslations> }) {
  return (
    <div className={styles.why_tabs__tab_content}>
      {/* eslint-disable-next-line @next/next/no-img-element */}
...
function TabPhilosophy({ t }: { t: ReturnType<typeof useTranslations> }) {
  return (
    <div className={styles.why_tabs__tab_content}>
      ...
      <Link href="/#home-form" className={styles.why_tabs__cta_btn}>
        {t('tab5.cta', {
